...
def get_sealevel_data(interpolation):
    """
    ...
    interpolation : string
        'Cubic' or 'linear'.  How to interpolate the sea level data.

    """

    path, _ = os.path.split(rc.__path__[0])
    ...
    every_year = np.arange(sldata[0, 0], sldata[-1, 0] + 1)
    sl = sl_interpolation_eq(every_year)
    d18o = d18o_interpolation_eq(every_year)

    sealevel_data = pd.DataFrame([every_year, sl, d18o], columns=header)
